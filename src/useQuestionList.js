...
// {
        ...
        groupContents: [{
            ...
            answerImg: [],
            answer: '3D',
            commentary: '視力が0.7(1.2×S+2.0D)の人が、約30cmの位置で本を読むとき必要なレンズの度数は3Dである。'
        },{
            detailInfo: '3.',
            questionImg: [],
            ...
            answerImg: [],
            answer: '外直筋',
            commentary: 'The abducens nerve controls the lateral rectus muscle.'
        },{
            detailInfo: '4.',
            questionImg: [],
            ...
            answerImg: [],
            answer: '網膜動脈閉塞症',
            commentary: 'Cataract is caused by retinal artery occlusion.'
        },{
            detailInfo: '5.',
            questionImg: [],
            ...
            answerImg: [],
            answer: '脈絡膜',
            commentary: 'The choroid is derived from the neural crest.'
        }]




// 3. Which muscle was controlled by the abducens nerve?
// a) 上直筋
// b) 外直筋
// c) 下直筋
// d) 上斜筋
// e)内直筋
// 4. Which disease causes the cataract?
// a) アトピー性皮膚炎
// b) 多発性硬化症
// c) 視神経炎
// d) 網膜動脈閉塞症
// e) 眼瞼下垂
// 5. Which is the characteristic antibody of the neuromyelitis optica?
// a)抗 SS-A 抗体
// b) 抗アクアポリン4抗体
// c) 抗甲状腺ペルオキシダーゼ抗体
// d) 抗核抗体
// e) 抗アセチルコリン受容体抗体
// 6. 動脈硬化を示唆する所見として最も適当なものを選べ。
// a) 硝子体混濁
// b) 視神経乳頭陥凹拡大
// c) 動静脈交叉現象
// d) 黄斑萎縮
// e) 角膜後面沈着物
// 7. 眼窩内に病変があった場合の症状として誤っているものを選べ。
// a) 眼球突出
// b)斜視
// c) 視力低下
// d) 飛蚊症
...
// d) 涙腺
// e) 網膜
// 9. 眼窩蜂窩織炎の時にみられる症状として誤っているものを選べ。
// a) 眼球突出
// b) 縮瞳
// c) 視力低下
// d) 眼瞼腫脹
// e) 眼球運動障害
// 10. 眼の解剖について正しいものを選べ。
// a) 眼窩で最も骨折しやすいのは上壁である。
// b) 視細胞の数について、 錐体細胞 桿体細胞=1:3である。
// c) 中心窩網膜に動脈は存在する。
// :
// d) 眼輪筋は顔面神経によって支配されている。
// e) 角膜内皮は障害されても再生する。
// 11. 角膜上皮について、誤っているものを選べ。
// a) 角膜全体の厚さの約10%を占める。
// b) 重層扁平上皮から構成されている。
// c) 一度障害されると原則再生されない。
// d) タイトジャンクションにより、 細胞間結合が強固である。
// e) 角膜知覚神経が豊富に存在する
// 12. 以下の組み合わせのうち、正しいものを選べ。
// a) Fabry 病-Kayser-Fleischer Ring
// b) Wilson 病ｰ渦状角膜
// c) 水疱性角膜症ｰ樹枝状角膜炎
// d) 角膜ヘルペス-輪状潰瘍
// e) Fuchs 内皮ジストロフィー滴状角膜
// 13. 次のうち、誤っているものを2つ選べ。
// a)斜視の精査では視力を始めとする眼科一般検査を原則として行わない。
// b) 美容的な要因で斜視の手術を行うことはない。
// c) 動眼神経麻痺では複視症状が生じることが多い。
// d)左上斜筋麻痺では右側に頭部を傾斜した頭位をとりやすい。
// e) スマートフォンを長時間見続けると内斜視になることがある。
// 14.兎眼の原因として誤っているものを選べ。
// a) 顔面神経麻痺
// b) 外傷性瘢痕
// c) 甲状腺眼症
// d) 動眼神経麻痺
// e) 眼窩腫瘍
// 15.急性緑内障発作について誤っているものを選べ。
// a)発作が起こっている眼は中等度散瞳している。
// b) 水晶体再建術は治療の一つである。
// c) 頭痛、嘔吐を伴うことがある。
// d) 服用している薬剤が発症の一因なる場合がある。
// e) 開放隅角眼に起こりやすい。
// 16. 常染色体優性視神経萎縮で正しいものを選べ。
// a) 自覚症状が強い。
// b) 視力障害の進行は早い。
// c) 眼底検査では片眼性の視神経萎縮を認める。
// d) 色覚障害は起こらない。
// e) 乳頭周囲の視神経線維層欠損を認める。
// 17. 次のうち、正しいものを選べ。
// a) 15 歳時に屈折異常弱視であることが判明したため積極的な治療が勧められた。
// b) 屈折異常弱視の治療として健眼遮蔽と眼鏡による完全屈折矯正を 24時間 6か月行った。
// c) 屈折異常弱視は近視眼で生じやすい。
// d) 乳児の瞳孔が白く見えたため早期の眼科受診を指示した。
// e) 斜視が原因で弱視になることはない。
// 18. 次のうち、正しいものを2つ選べ。
// a) 日本では視覚障害の基準を満たした時点で障害者手帳が交付される。
// b) 緑内障は日本における中途視覚障害・失明原因の第1位である。
// c) 40歳以上の緑内障の有病率は15%といわれている。
// d) 糖尿病網膜症は日本における中途視覚障害・失明原因の第2位である。
// e) 白杖は周囲に視覚障害であることを認識してもらうことにも役立つ。
